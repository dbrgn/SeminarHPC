...
__kernel void crack(__global const unsigned char *hash, __global char *result, const unsigned char len)
{
    ...
    // Debug output
    __private int ids[3] = { 9999, 9999, 9999 };

    for (int i = 0; i < 3; i++) {
        // Hack: Exit early if length is too short.
        if (len <= i) break;
    ...
    // Debug output
    /*
    __private char out[len + 1];
    for (int i = 0; i < len; i++) out[i] = string[i];
    out[len] = '\0';
    printf(">>> [GlobalID %d,%d,%d] Testing '%s'...\n", ids[0], ids[1], ids[2], out);
    */
