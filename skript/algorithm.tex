...
alle ersten partiellen Ableitungen sind.
...
Dann ist der Vektor $x\in\mathbb R^n$ genau dann ein Minimum von
...
also auch versuchen, das zugeh"orige Minimalproblem zu l"osen. 
...
korrigiere $x_{i+1}= x_i + sv$.
\item Wiederhole \ref{descent:loop0} so lange,
bis $|J(x_{i+1})-J(x_i)|<\varepsilon$.
\end{enumerate}
In diesem Algorithmus ist der Wert von $s$ nicht definiert.
Am besten wird $t$ so gew"ahlt, dass $x_{i+1}$ von allen m"oglichen $t$
den kleinstm"oglichen Wert ergibt.
Wir berechnen daher 
\begin{align*}
J(x_i+sv)
&=
\frac12(x_i+sv)^tA(x_i+sv)-(x_i+sv)^tb
\\
&=
\frac12x_i^tAx_i -x_i^tb + s\cdot \biggl(\frac12v^tAx_i +\frac12x_i^tAv-v^tb\biggr) + s^2\cdot \frac12v^tAv
\\
&=
J(x_i)+s\cdot v^t(Ax_i-b) +s^2\cdot \frac12v^tAv
\end{align*}
Eine quadratische Funktion $s\mapsto as^2+bs+c$ hat ihr Minimum an der
Stelle $s=-\frac{b}{2a}$. 
Im vorliegenden Fall f"uhrt dies auf die optimale Wahl f"ur $s$
\[
s=-\frac{v^t(Ax_i-b)}{v^tAv}.
\]
Setzen wir jetzt wieder $v=\operatorname{grad}J(x_i)=Ax_i-b$ ein, erhalten
wir
\[
s=-\frac{v^tv}{v^tAv}.
\]
Damit k"onnen wir den Algorithmus des steilsten Abstiegs jetzt ausformulieren:
\begin{enumerate}
\item W"ahle eine beliebige Anfangsapproximation $x_1$ der L"osung
\item Berechne den Gradienten $v=Ax_i-b$ von $J$ an der Stelle
$x_i$
\item Berechnen 
\begin{equation}
x_{i+1}=x_i -\frac{v^tv}{v^tAv}v.
\label{descent:iteration}
\end{equation}
\item Wiederhole Schritte 2 und 3 bis $|x_{i+1}-x_i|<\varepsilon$.
\end{enumerate}
Dies ist ein Beispiel eines Abstiegsalgorithmus.
...
Die Abbildung~\ref{descent:path} zeigt den Pfade des steilsten Abstiegs.
In diesem zweidimensionalen Fall ist sofort erkennbar, wie dass der
Pfadselbst"ahnlich ist, die Genauigkeit wird also in jedem Iterationsschritt
...
\subsubsection{Abstieg f"ur eine Kugel}
F"ur dieses Beispiel wollen wir von $A=\operatorname{diag}(a_1,\dots,a_n)$
und $b=0$ ausgehen.
...
Richtungen $v_i$ abnehmen.
Ein solche orthonormierte Basis kann mit dem Orthognalisierungsverfahren
von Gram-Schmidt ausgehend von einer beliebigen Vektorfolge konstruiert
werden.

\subsubsection{Das Skalarprodukt zu $A$}
Das gew"ohnliche Skalarprodukt wurde f"ur die Definition einer Folge von
Vektoren verwendet, um die Folge von Richtungen $v_i$ zu konstruieren.
Nat"urlich funktioniert das nur f"ur $A=E$, also f"ur eine Kugel als 
Niveaufl"ache der Funktion $J(x)$.

Die Matrix $A$ definiert auch ein Skalarprodukt
\[
...
\]
Sei $x_0$ die gesuchte L"osung, also $Ax_0=b$.
Dann gilt
\begin{align*}
\langle x-x_0,x-x_0\rangle_A
&=
(x-x_0)^tA(x-x_0)
\\
&=x^tAx-2x^tAx_0+x_0^tAx_0
\\
&=\langle x,x\rangle_A -2x^tb+x_0^tb
\\
&=2J(x)+x_0^tb
\end{align*}
Die L"osung wird also gefunden als das Minimum einer quadratischen Form
$\langle x-x_0,x-x_0\rangle_A$.
Verwendet man das von $A$ erzeugte Skalarprodukt zur L"angenbestimmung,
dann sind die Niveaufl"achen Kugeln, und es kann eine beliebige Folge
von orthonormierten Vektoren f"ur den Abstieg verwendet werden.
Allerdings muss Orthogonalit"at ebenfalls im Sinne des Skalarprodukts
$\langle\cdot ,\cdot \rangle_A$ gew"ahlt werden.

\begin{beispiel}
Wir wollen einmal mehr das Gleichungssystem mit Matrix
\[
A=\begin{pmatrix}
1&\frac12\\\frac12&1
\end{pmatrix}
...
b=\begin{pmatrix}1\\-1\end{pmatrix}
\]
l"osen. Ausgehend vom Punkt $(1,0)$ bestimmen wir jetzt eine Folge
von Vektoren.
Als ersten Vektor verwenden wir wieder den Gradienten an dieser 
Stelle:
\begin{align*}
x_1&=\begin{pmatrix}1\\0\end{pmatrix},
&
v_1&=Ax_1-b=\begin{pmatrix}0\\\frac32\end{pmatrix},
&
|v_1|_A&=\frac32,
&
v_1'&=\frac1{|v_1|_A}v_1=\begin{pmatrix}0\\1\end{pmatrix}
\end{align*}
\begin{align*}
x_2&=x_1-\frac{\langle v_1',Ax_1-b \rangle_A}{\langle v_1',v_1'\rangle_A}v_1'=
\begin{pmatrix}1\\0\end{pmatrix}-\frac32\begin{pmatrix}0\\1\end{pmatrix}
=
\begin{pmatrix}1\\-\frac32\end{pmatrix}.
\end{align*}
Jetzt muss die n"achste Richtung bestimmt werden.
Wir m"ochten dazu zwar wieder $Ax_2-b$ verwenden, davon aber nur die Komponente
senkrecht auf $v_1$, also
\begin{align*}
v_2&=Ax_2-b=\begin{pmatrix}-\frac34\\0\end{pmatrix},
&
v_2'&=v_2-\langle v_1,v_2\rangle_A v_1
=\begin{pmatrix}-\frac34\\-1\end{pmatrix}
%&
%v_2'
%&=\frac{v_2-\langle v_1,v_2\rangle_A v_1}{|v_2-\langle v_1,v_2\rangle_A v_1|}_A
%=
\end{align*}
Jetzt erfolgt wieder ein Abstiegsschritt in Richtung $v_2'$
\begin{align*}
